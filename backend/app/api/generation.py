...
def _inventory_rocs(db: Session) -> list | None:
    ...
    rows = (
        ...
        .filter(InventoryItem.category.in_(("mirror", "lens")))
        .distinct()
        .all()
    )
...
def _full_case_data(case: ExperimentCase, *, with_physics: bool = False,
    ...
    if with_physics:
        ...
        data["computed_physics"] = compute_case_physics(
            case.parameters, available_rocs=_inventory_rocs(db) if db is not None else None
        )
    return data
...
async def generate_plan(
    ...
    content, latency_ms = await _run_generation(
        "plan", provider.generate_plan(_full_case_data(case, with_physics=True, db=db))
    )
    content["computed_physics"] = compute_case_physics(
        case.parameters, available_rocs=_inventory_rocs(db)
    )
